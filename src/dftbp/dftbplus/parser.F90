...
#:endif
  ...
  !> Reads the SSR(4,4) block
  subroutine readSSR44(node, ctrl, geo)
    ...
    !> Decide the energy functionals to be included in SA-REKS(4,4)
    nFunc = size(tmpFunc, dim=1)
    if (nFunc == 1) then
      if (trim(tmpFunc(1)) == "PPS") then
        !> Minimized energy functional : PPS
      ...
      if (trim(tmpFunc(1)) == "PPS" .and. trim(tmpFunc(2)) == "DSPS") then
        !> Minimized energy functional : (PPS+DSPS)/2
        ctrl%reksInp%Efunction = 2
      else
        tFunc = .false.
      end if
